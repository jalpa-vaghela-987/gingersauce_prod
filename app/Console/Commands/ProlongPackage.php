...
class ProlongPackage extends Command {
    ...
    public function handle() {
        ...
        info('ProlongPackage: ', [$packages_to_prolong]);
        ...
        if ( $failed_attempts->isNotEmpty() ) {
            ...
            foreach ( $failed_attempts as $failed_attempt ) {
                ...
                if ( !empty( $user ) ) {
                    ...
                    info('Prolong(payOnFailed) foreach before continue: ', [$user, $package]);
                    if(!$package) continue;
                    info('Prolong(payOnFailed) foreach after continue: ', [$user, $package]);
                    ...
                    if ( !empty( $invoice ) ) {
                        ...
                        if ( $ps_user[ 'status' ] ) {
                            info( 'Prolong(payOnFailed) from if of $ps_user ', [$ps_user] );
                            ...
                            if ( config( 'app.env' ) !== 'local' ) {
                                info('Prolong(payOnFailed) send CheckoutInfo mail');
                                Mail::to( 'mor.alon@gingersauce.co' )->send( new CheckoutInfo( $invoice ) );
                            }

                            info( 'Prolong(payOnFailed) Creating Bill' );

                            $bill_result = $this->createBill( $ICount, $invoice );

                            info( 'Prolong(payOnFailed) Result ', [$bill_result] );

                            if ( config( 'app.env' ) !== 'local' ) {
                                info('Prolong(payOnFailed) send CheckoutResult mail');
                            ...
                            if ( $bill_result[ 'status' ] === true ) { // bill created
                                try {
                                    ...
                                    info( 'Prolong(payOnFailed) catch message on status of created bill is true', [$e->getMessage()] );
                                    info( 'Prolong(payOnFailed) catch trace on status of created bill is true', [$e->getTraceAsString()] );
                                }
                                ...
                                if ( $doc[ 'status' ] == true ) {
                                    ...
                                    info( 'Prolong(payOnFailed) Failed on creating doc ' . var_export( $doc, true ) );
                                }
                                ...
                                info('Prolong(payOnFailed) send FailedPaymentPackage mail', [$bill_result]);
                            ...
                            info( "Prolong(payOnFailed) Unable load user ", [$ps_user] );
                        }
                    }
                    else{
                        info( "Prolong(payOnFailed) Failed to create invoice ", [$invoice] );
                    }
                }
            }
        }
        else{
            info('Prolong(payOnFailed) Does not have users for failed_attempts: ', [$failed_attempts]);
        }
    }
    ...
    protected function prolong( Collection $packages_to_prolong ) {
        if ( $packages_to_prolong->isNotEmpty() ) {
            ...
            foreach ( $packages_to_prolong as $item ) {
                ...
                info('Prolong foreach before continue: ', [$user, $package]);
                if(!$package) continue;

                info('Prolong foreach after continue: ', [$user, $package]);
                ...
                if ( !empty( $invoice ) ) {
                    ...
                    if ( $ps_user[ 'status' ] ) {
                        info( 'From if of $ps_user ', [$ps_user] );
                        ...
                        if ( config( 'app.env' ) !== 'local' ) {
                            info('Prolong send CheckoutInfo mail');
                            Mail::to( 'mor.alon@gingersauce.co' )->send( new CheckoutInfo( $invoice ) );
                        }

                        info( 'Prolong: Creating Bill' );

                        $bill_result = $this->createBill( $ICount, $invoice );

                        info( 'Prolong: Result ', [$bill_result] );
                        if ( config( 'app.env' ) !== 'local' ) {
                            info('Prolong send CheckoutResult mail');
                            Mail::to( 'mor.alon@gingersauce.co' )->send( new CheckoutResult( $invoice, $bill_result ) );
                        }
                        ...
                        if ( $bill_result[ 'status' ] === true ) { // bill created
                            try {
                                ...
                                info( 'Prolong catch message on status of created bill is true', [$e->getMessage()] );
                                info( 'Prolong catch trace on status of created bill is true', [$e->getTraceAsString()] );
                            }
                            ...
                            if ( $doc[ 'status' ] == true ) {
                                ...
                                info( 'Prolong Failed on creating doc ' . var_export( $doc, true ) );
                            }
                            ...
                            info('Prolong send FailedPaymentPackage mail', [$bill_result]);
                            Mail::to( $user->email )->bcc( 'payments@gingersauce.co' )->send( ( new FailedPaymentPackage() ) );
                        ...
                        info( "Prolong Unable load user ", [$ps_user] );
                    }
                }else{
                    info( "Prolong Failed to create invoice ", [$invoice] );
                }

            }
        }else{
            info('Prolong Does not have users for recurring payment: ', [$packages_to_prolong]);
        }
    }
    ...
    protected function createDoc( ICount $ICount, Invoice $invoice, $bill_result, $client_name ) {
        ...
        if ($invoice->vat > 0) {
            $data['vat_percent'] = $invoice->vat;
        } else {
            $data['tax_exempt'] = true;
        }

        //info('ProlongPackage: doc_req_data; ', $data);
