...
class InvoiceController extends Controller {
    ...
    public function checkout( Invoice $invoice, Request $request, ICount $ICount ) {
        ...
        info( 'InvoiceController Checkout method: Creating Bill' );
        $bill_result = $this->createBill( $request, $ICount, $invoice->description, $total, $client_id );

        info( 'InvoiceController Checkout method Result: ', $bill_result );
        ...
        if ( $bill_result[ 'status' ] === true ) { // bill created
            ...
            try {
                ...
                //info('From InvoiceController Doc: ', ['doc' => $doc]);
                if ( $doc[ 'status' ] == true ) {
    ...
    protected function createDoc( Request $request, Invoice $invoice, ICount $ICount, $total, $result ) {
        ...
        if ($invoice->coupon_id) {
            $discount = $invoice->price * ($invoice->coupon->discount / 100);
            $items[] = [
                'description' => 'Coupon Discount (' . $invoice->coupon->coupon . ') ' . $invoice->coupon->discount . '%',
                'unitprice'   => -$discount,
                'quantity'    => 1,
            ];
        }
    ...
    protected function storeCard( Request $request, ICount $ICount, $client_id ) {
        ...
        if ( $token_info[ 'status' ] ) {
            ...
            $cc_stored_user_card
                ...
                'cc_card_type'     => $token_info[ 'cc_holder_name' ],
