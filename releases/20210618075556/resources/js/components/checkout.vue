<template>
    <div>
        <div v-if="!using_token">
            ...
            <div class="bg-white rounded p-4">
                ...
                <div class="form-group">
                    ...
                    <input name="state" id="invoice-co-state" type="text" class="form-control underlined"
                           v-model="state">
                </div>
                <div class="form-group required">
        ...
        <input type="hidden" name="invoice_id" v-model="id">

        <h1 class="">
            {{ translate('Payment method') }}
        </h1>
        <div class="bg-white rounded p-4 mb-5" v-if="!using_token">
            <div class="form-group required">
            ...
            <div class="form-group required" v-if="country == 'IL'">
                ...
                <input name="id_on_card" required id="invoice-co-id-on-card" type="text" class="form-control underlined"
                       v-model="id_on_card">
            </div>
            <div class="form-group required">
                <label for="invoice-co-cc">{{ translate('Credit card number') }}</label>
                <input name="cc" id="invoice-co-cc" required type="text" class="form-control underlined" v-model="cc">
                ...
                <div class="col-4">
                    <div class="form-group required">
                        ...
                        <input name="cvc" required id="invoice-co-cvc" maxlength="4" type="password"
                               class="form-control underlined" v-model="cvc">
                        <div class="img-description">
        ...
        <div class="bg-white rounded p-4" v-if="using_token">
            <label class="vue-checkbox-container">{{ translate('Use payment info on file') }}
                <input type="checkbox" v-model="using_token" name="using_token" value="1">
                <span class="checkmark"></span>
            </label>
            <div class="stored-card-field" style="position:relative;">
                <div class="stored-card">
                    XXXX-XXXX-XXXX-{{ last_4 }}
                </div>
                <i class="payment-card-icon" :class="card_type"></i>
            </div>
        </div>
    </div>
</template>
...
export default {
    ...
    watch: {
        country: function (val) {
            this.$root.$emit('country-change', {country: val});
        },
        using_token: {
    ...
    methods: {
        translate,
        load() {
            axios.post(endpoints.ajax.post.load_user, {id: this.id, user_id: this.user_id})
                .then(response => {
