<template>
  <div class="wrapper-default-block">
    ...
    <SubMenuDownload :elements="elements"
                     @download="handleMockupOneDownload"
                     @copy="handleMockupOneCopy"/>
    <div class="default-block d-flex align-items-center justify-content-center mockup-block">
      <img :src="imgSrc" alt="Mockup Image" />
    </div>
  </div>
</template>
<script>
    ...
    export default {
        ...
        methods: {
            async handleMockupOneDownload(format) {
                await this.downloadFile(this.imgSrc, format);
            },
            async handleMockupOneCopy(format) {
