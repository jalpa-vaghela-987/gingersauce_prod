    ...
    <div class="col-4 font-upload font-block-container" :class="[(!previous_uploaded) ? 'font-block-dis' : '', (mobile_index == index) ? 'mobile-show' : '']">
        <div :class="['font', font_file_uploaded!=''?'uploaded':'']" :style="{'--color': background_color}">
            ...
            <div class="font-dropdown-wrapper" v-bind:class="{ opened: show_font_dropdown }">
                <div class="search">
                    <input type="text" v-model="gfont_search" :placeholder="translate('search')"/>
                ...
                <div class="font-dropdown" id="primary-font-dropdown">
            ...
            <div v-if="(font_file_uploaded=='' && index!=1 && gfont_fin_selected) || (font_file_uploaded=='' && index==1 && gfont_selected)" class="g-font-block second fin"
                 @click.stop.prevent=toggleFontDropDownWeight>
        ...
        <small>{{project_title}}</small></div>
            <div class="optional" v-if="optional && font_face_weight == ''">{{ translate('Optional') }}</div>
        </div>
        <div class="font-block-title" v-if="font_file_uploaded==''">
                {{ translate('Font Weight') }} {{translate(index)}}</div>
...
<script>
        ...
        export default {
                ...
                mounted()
                {
                        ...
                        const apiKeyMeta = document.querySelector('meta[name=google-fonts-key]');
                        if (!apiKeyMeta) {
                            console.error('Google Fonts API key meta tag not found');
                            return;
                        }
                        this.apikey = apiKeyMeta.content;
                        console.log('Font.vue API Key loaded:', this.apikey ? 'Yes' : 'No');

                        if (!this.isEmpty(this.fontdata))
                        {
                                console.log('Font.vue Font data exists:', this.fontdata);
                        ...
                        {
                                console.log('Font.vue No font data, loading Google Fonts...');
                                this.load()
                        }

                        // Listen for google-fonts-loaded event
                        this.$root.$on('google-fonts-loaded', (data) => {
                            console.log('Font.vue received google-fonts-loaded event:', data);
                            if (data.fonts) {
                                this.google_fonts = data.fonts;
                                this.visibles = this.google_fonts.slice(0, 100);
                                console.log('Font.vue updated fonts list:', this.google_fonts.length);
                            }
                        });
                ...
                watch      : {
                        font_file_uploaded()
                        {
                                if (this.font_file_uploaded != '')
                                {
                                        // Set the font face name immediately if it's a Google Font
                                        if (this.font_file_uploaded.includes('fonts.gstatic.com')) {
                                                this.font_face_name = this.fontdata.font_face;
                                                this.font_face_weight = this.fontdata.weight;
                ...
                methods    : {
                        ...
                        selectGfontWeight(w)
                        {
                                // Convert weight to Google Fonts format
                                let weight = w;
                                if (w === 'regular') weight = '400';
                                if (w === 'italic') weight = '400i';
                                if (w.includes('italic')) {
                                    weight = w.replace('italic', 'i');
                                }

                                // Create Google Fonts URL
                                const fontUrl = `https://fonts.googleapis.com/css2?family=${this.gfont.family.replace(/ /g, '+')}:wght@${weight}&display=swap`;
                                ...
                                this.font_file_uploaded = fontUrl;
                                this.font_face_weight = w;
                                this.font_face_name = this.gfont.family;

                                this.$root.$emit('gfont-fin', {
                                        gfont  : this.gfont,
                                        weight : w
                                });
                                this.$root.$emit('gfont-font-next');

                                // Use the same event as upload
                                this.$root.$emit('font.load', {
                                        type      : this.type,
                                        index     : this.index,
                                        file      : fontUrl,
                                        font_face : this.gfont.family,
                                        weight    : w
                                });
                        },
                        ...
                        load()
                        {
                                ...
                                if (!this.apikey) {
                                    console.error('Font.vue Google Fonts API key is missing');
                                    return;
                                }
                                ...
                                axios.get(apiUrl)
                                    .then(response => {
                                        console.log('Font.vue Google Fonts API Response:', response.data);
                                        if (response.data && response.data.items) {
                                            this.google_fonts = response.data.items;
                                            // Update visibles array with first 100 fonts
                                            this.visibles = this.google_fonts.slice(0, 100);
                                            console.log('Font.vue Loaded fonts:', this.google_fonts.length);
                                            console.log('Font.vue Visible fonts:', this.visibles.length);

                                            // Emit event to notify that fonts are loaded
                                            this.$root.$emit('google-fonts-loaded', {
                                                fonts: this.google_fonts
                                            });
                                        } else {
                                            console.error('Font.vue Invalid response format from Google Fonts API');
                                        }
                                    })
                                    .catch(error => {
                                        console.error('Font.vue Error loading Google Fonts:', error);
                                        if (error.response) {
                                            console.error('Font.vue Error response:', error.response.data);
                                        }
                                    });
                        },
                        searchFont()
                        {
                                ...
                                this.visibles = this.google_fonts.filter(font =>
                                    font.family.toLowerCase().includes(val)
                                );
                        },
                        isEmpty(obj)
                        {
                        ...
                        input_file(newFile, oldFile)
                        {
                                if (newFile && !oldFile)
                                {
                                        ...
                                        // Try to read the font file directly for preview only
                                        ...
                                        reader.onload = (e) => {
                                                try {
                                                        ...
                                                        // Store font metadata temporarily
                                                        this.tempFontMetadata = {
                                                            font_face: font.names.preferredFamily ? font.names.preferredFamily.en :
                                                                     font.names.fontFamily ? font.names.fontFamily.en : '',
                                                            weight: font.names.preferredSubfamily ? font.names.preferredSubfamily.en :
                                                                   font.names.fontSubfamily ? font.names.fontSubfamily.en : ''
                                                        };

                                                        // Load the font using FontFace API for preview only
                                                        var new_font = new FontFace(this.font_title, 'url(' + tempUrl + ')')
                                                        new_font.load().then((face) => {
                                                                document.fonts.add(face);
                                                        }).catch(error => {
                                                                ...
                                                                new Noty({
                                                                    type: 'error',
                                                                    text: this.translate('Error loading font. Please try another file.'),
                                                                    timeout: 5000,
                                                                }).show();
                                                        });
                                                } catch (error) {
                                                        ...
                                                        new Noty({
                                                            type: 'error',
                                                            text: this.translate('Invalid font file. Please try another file.'),
                                                            timeout: 5000,
                                                        }).show();
                                                }
                                        };
                                ...
                                if (newFile && oldFile)
                                {
                                        ...
                                        if (newFile.success !== oldFile.success)
                                        {
                                                ...
                                                const storagePath = newFile.response.font;
                                                const publicUrl = storagePath.replace('storage/app/public', 'storage');

                                                // Use the public URL
                                                this.font_file_uploaded = publicUrl;
                                                this.font_face_name = this.tempFontMetadata.font_face;
                                                this.font_face_weight = this.tempFontMetadata.weight;

                                                // Load the font from the public URL
                                                this.loadFontFromUrl(publicUrl);

                                                // Emit font load event with the public URL
                                                this.$root.$emit('font.load', {
                                                    type: this.type,
                                                    index: this.index,
                                                    file: publicUrl,
                        ...
                        processFontMetadata(f) {
                                ...
                                } else if (f.names.fontSubfamily != undefined) {
                        ...
                        loadFontFromUrl(fontUrl) {
                                var new_font = new FontFace(this.font_title, 'url(' + fontUrl + ')')
                                new_font.load().then((face) => {
                                        document.fonts.add(face)
                                }).catch(error => {
                                        console.error('Error loading font face:', error);
                                        // Try loading without protocol
                                        let fallbackUrl = fontUrl.replace(/^https?:\/\//, '//');
                                        var fallback_font = new FontFace(this.font_title, 'url(' + fallbackUrl + ')')
                                        fallback_font.load().then((face) => {
                                                document.fonts.add(face)
                                        }).catch(error => {
                                                console.error('Error loading font face with fallback:', error);
                                        });
                                });

                                opentype.load(fontUrl, (e, f) => {
                                        if (e) {
                                                // Try loading without protocol
                                                let fallbackUrl = fontUrl.replace(/^https?:\/\//, '//');
                                                opentype.load(fallbackUrl, (e2, f2) => {
                                                        if (e2) {
                                                                console.error('Error loading font with opentype fallback:', e2);
                                                        } else {
                                                                this.processFontMetadata(f2);
                                                        }
                                                });
                                        } else {
                                                this.processFontMetadata(f);
                                        }
                                });
                        },
                }
