...
<script>
        ...
        export default {
                ...
                mounted()
                {
                        ...
                        if (!apiKeyMeta) {
                            console.error('Font_s.vue Google Fonts API key meta tag not found');
                            return;
                        }
                        this.apikey = apiKeyMeta.content;
                        console.log('Font_s.vue API Key loaded:', this.apikey ? 'Yes' : 'No');

                        if (!this.isEmpty(this.fontdata))
                        {
                                console.log('Font_s.vue Font data exists:', this.fontdata);
                ...
                methods    : {
                        ...
                        load()
                        {
                                ...
                                if (!this.apikey) {
                                    console.error('Font_s.vue Google Fonts API key is missing');
                                    return;
                                }
                                ...
                                axios.get(apiUrl)
                                    .then(response => {
                                        console.log('Font_s.vue Google Fonts API Response:', response.data);
                                        if (response.data && response.data.items) {
                                            this.google_fonts = response.data.items;
                                            this.visibles = this.google_fonts.slice(0, 100);
                                            console.log('Font_s.vue Loaded fonts:', this.google_fonts.length);
                                            ...
                                            console.error('Font_s.vue Invalid response format from Google Fonts API');
                                        }
                                    })
                                    .catch(error => {
                                        console.error('Font_s.vue Error loading Google Fonts:', error);
                                        if (error.response) {
                                            console.error('Font_s.vue Error response:', error.response.data);
                                        }
                                    });
                        },
                        searchFont()
                        ...
                        input_file(newFile, oldFile)
                        {
                                if (newFile && !oldFile)
                                {
                                        ...
                                        // Try to read the font file directly
