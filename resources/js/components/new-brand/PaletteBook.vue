<template>
  <section class="align-self-start col-12 section-book" id="palette-book" :style="`order:${orderClass}`">
    <div class="content">
        <span v-if="isEditBook">
            <EditDefaultInput
                :value="(brandbook.pallette_title) ? brandbook.pallette_title : 'Our Color Palette'"
                filed="pallette_title"
            />
            <EditDefaultDescription
                :description="(brandbook.pallette_text) ? brandbook.pallette_text : setDefaultDescription"
                filed="pallette_text"
            />
        </span>
        <span v-else>
            <h1 class="mb-4">{{ (brandbook.pallette_title) ? brandbook.pallette_title : 'Our Color Palette' }}</h1>
            <span v-if="brandbook.pallette_text" v-html="brandbook.pallette_text"></span>
            <span v-else v-html="setDefaultDescription"></span>
        </span>
        <WatermarkImage/>
    </div>
    <div class="palette-group d-flex flex-wrap" v-if="defaultBrandbook && defaultBrandbook.colors_list">
      <PaletteElementTertiary class="align-self-end mb-3 position-absolute z-1"  @updateBrandbookColorList="updateBrandbookColorList" />
      <PaletteElementMain class="ms-0 ms-md-5" @updateBrandbookColorList="updateBrandbookColorList"/>
      <PaletteElementSecond class="align-self-end mb-5" @updateBrandbookColorList="updateBrandbookColorList" />
    </div>
    <div class="col-10">
      <p>
    ...
    <div class="pantone-group d-flex flex-wrap">
      <AddPantone
          v-for="(colors, index) in brandbook.colors_list"
          :key="index"
          :colorList="colors"
          :position="index"
          @updateBrandbookColorList="updateBrandbookColorList"
      />
    </div>
...
export default {
    data(){
        return {
            ...
            primary_color     : this.brandbook?.colors_list?.[0]?.id,
            secondary_color   : this.brandbook?.colors_list?.[1]?.id,
            tetriatry_color   : this.brandbook?.colors_list?.[2]?.id,
    ...
    components:{
        PaletteElementMain,
        PaletteElementSecond,
        PaletteElementTertiary,
        SimplePantone,
        AddPantone,
        EditDefaultDescription,
        EditDefaultInput,
    ...
    mounted(){
        this.getOrderClass('color-palette');
    },
    methods : {
        ...mapActions([
            'setDefaultBrandbook',
        ]),
        updateBrandbookColorList(index, colorList) {
            this.brandbook.colors_list[index].id = colorList.hex.value;
            this.brandbook.colors_list[index].color = colorList;
            this.updateStateColorList(this.brandbook.colors_list);
        },
        updateStateColorList(colorList) {
            this.setDefaultBrandbook({
                ...this.defaultBrandbook,
                'colors_list' : colorList
            });
            this.updateApprovedIcons();
            this.updateLogoVersions();
        },
        updateApprovedIcons() {
            let approvedIcons = [];
            this.brandbook.approved_icon.map((approvedIcon, index) => {
                if(approvedIcon.title == 'White Negative on Primary in Square' || approvedIcon.title == 'White Negative on Primary in Circle') {
                    approvedIcon.background = this.defaultBrandbook.colors_list[0].id
                } else if(approvedIcon.title == 'White Negative on Secondary in Square' || approvedIcon.title == 'White Negative on Secondary in Circle') {
                    approvedIcon.background = this.defaultBrandbook.colors_list[1].id
                } else if(approvedIcon.title == 'White & Colors on Secondary in Square' || approvedIcon.title == 'White & Colors on Secondary in Circle') {

                }

                approvedIcons.push(approvedIcon);
            })
            this.setDefaultBrandbook({
                ...this.defaultBrandbook,
                'approved_icon' : approvedIcons
            });
        },
        updateLogoVersions() {
            this.primary_color     = this.defaultBrandbook.colors_list[0].id;
            this.secondary_color   = this.defaultBrandbook.colors_list[1].id;
            this.tetriatry_color   = this.defaultBrandbook.colors_list[2].id;
            this.logo_variations = this.brandbook.approved;

            this.logo_variations = this.applyVariations();
            ...
            this.setDefaultBrandbook({
                ...this.defaultBrandbook,
                'approved' : this.logo_variations
            });
        },

        applyVariations() {
            var result = this.list().map( ( params, index ) => (params.isChange) ? this.applyVariation( params, params, params.id.split("_")[0] ) : params.logo_variations );
            return result;
        },
        ...
        list() {
            ...
            this.logo_variations.map((logoVariation, index) => {
                    let listdata = list.find(o => o.title == logoVariation.title);
                    let notChanged = ['Black & White Positive', 'White Negative', 'White & Black', 'Black & White Negative', 'White & Colors', 'Positive on Black'];
                    listdata.logo = logoVariation.logo;
                    listdata.id = logoVariation.id;
                    listdata.title =  logoVariation.title;
                    listdata.isChange = (notChanged.indexOf(logoVariation.title) == '-1') ? true : false;
                    listdata.logo_variations = logoVariation;
                    lists.push(listdata);
            })
            return lists;
        },
    ...
    computed: {
        ...
        setDefaultDescription() {
            return `<p>The colors selected for the ${this.brandbook.brand} signature reflect the company's values.<br /> The colors have been specifically chosen to represent the brand and should not be altered under any circumstance.<br /> For Printing instances, a Rich Black should be used for text with C40 M10 Y0 K100.</p>`
        },
    },
    watch: {
        currentBrandTabs: {
...
.palette-group ,.palette-group +.content{
    z-index:  2 !important;
}
</style>
