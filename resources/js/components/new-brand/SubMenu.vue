<template>
  <div class="nav-item dropdown" :class="{ show: showDropdown }">
    ...
    >
      <RoundedImage style="width: 34px; height: 34px" :image="user.avatar_url"/>
      ...
      >
        ...
        <li>
          <a :href="endpoints.ajax.get.header.plans_pricing" class="dropdown-item">
              <i class="fas fa-tag"></i> Plans & Pricing
          </a>
        </li>
        <li>
            <a :href="endpoints.ajax.get.header.profile" class="dropdown-item">
                <i class="fa fa-user"></i> My Profile
            </a>
        </li>
        <li>
            <a :href="endpoints.ajax.get.header.plans" class="dropdown-item">
                <i class="fa fa-user"></i> Plans
            </a>
        </li>
        <li>
            <a :href="endpoints.ajax.get.header.billing" class="dropdown-item">
                <i class="fa fa-credit-card"></i> Billing
            </a>
        </li>
        <li>
            <a :href="endpoints.ajax.get.header.purchase_history" class="dropdown-item">
        ...
        <li class="nav-item dropdown" :class="{ show: showSubmenu }">
          ...
          >
              <li v-for="(locale, index) in localeUrls"
                  :key="index">
                  <a class="dropdown-item" :href="locale.url">{{ locale.name }}</a>
              </li>
          </ul>
        </li>
        <li>
...
export default {
    ...
    computed: {
        ...mapState(['canRemoveWm', 'localeUrls', 'isEditBook'])
    },
  props:{
    user:{
      type:Object,
