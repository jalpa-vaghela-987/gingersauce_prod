...
@section('content')
    <div class="content" style="text-align: center; padding: 50px 0 0 50px;">
        ...
        <a href="@if($brandbook->bb_version==1) {{ config('app.url'). "/my/view/".$brandbook->id }} @elseif($brandbook->bb_version==2)  {{ config('app.url'). "/my/view-new/".$brandbook->id }} @else {{ config('app.url'). "/my/view-new-v3/".$brandbook->id }} @endif" class="change-pm-button" style="text-decoration: none;    color: white;    background: #29B473;    padding: 10px 50px;    border-radius: 5px;    margin-top: 20px;    display: inline-block;">View Brand Book</a>
