<div class="brandbook-block col-3 p-3">
    <div class="contents p-3 rounded-16">
...
@php
    if($brandbook->bb_version == 1){
        $url = route('brandbook.my').'/view/'.$brandbook->id;
    } elseif($brandbook->bb_version == 2) {
        $url = route('brandbook.my').'/view-new/'.$brandbook->id;
    } else {
        $url = route('brandbook.my').'/view-new-v3/'.$brandbook->id;
    }
@endphp
        <a class="image" data-href="{{ $url }}"
           href="{{($brandbook->completed>=20) ? $url : route('brandbook.my').'/resume/'.$brandbook->id}}"
           style="position: relative;background-image: url(@if($brandbook->finished()){{($brandbook->bb_version == 1) ? route('pdf_preview', $brandbook) : $brandbook->logo_b64 }}@else{{$brandbook->logo_b64}}@endif);
           @if($brandbook->finished() && $brandbook->bb_version ==1 )padding-top: calc(100% + 1rem); margin: -1rem !important; margin-bottom: 0rem !important;background-size: cover;@endif">
        ...
        <div class="bottom-content d-flex justify-content-between">
            <div class="texts">
                <div class="title font-weight-boldic mt-2 @if($brandbook->user_id!=Auth::user()->id) text-muted @endif">
                    <a data-href="{{ $url }}"
                       href="{{($brandbook->completed>=20) ? $url : route('brandbook.my').'/resume/'.$brandbook->id}}">{{$brandbook->brand ?: 'New Brandbook'}}</a>
                </div>
                @if($brandbook->user_id == Auth::user()->id)
                    <div class="last-updated small text-muted">
                        <a data-href="{{$url}}"
                           href="{{($brandbook->completed>=20) ? $url : route('brandbook.my').'/resume/'.$brandbook->id}}">{{__('Last updated')}} {{$brandbook->human_date()}}</a>
                    </div>
                @else
                    <div class="last-updated small ">
                        <a data-href="{{$url}}"
                           href="{{($brandbook->completed>=20) ? $url : route('brandbook.my').'/resume/'.$brandbook->id}}">{{__('Handed ')}} {{$brandbook->human_date()}}</a>
                    </div>
                @endif
                @if(!empty($brandbook->expires_at) && 1==2) <!-- used this condition to remove -->
            ...
            @endif
            <div class="dropup">
                <button type="button" class="btn btn-default pr-0 bg-transparent bb-dropdown" data-toggle="dropdown">
...
{{--                    @endif--}}
                    @if(Auth::user()->can_do('edit', $brandbook))
                        @if($brandbook->completed>=20 && $brandbook->completed != 121)
                    ...
                    @if($brandbook->completed>=20)
                        ...
                        @elseif($brandbook->bb_version == 2)
                            <a class="dropdown-item" href="{{route('brandbook.my')}}/view-new/{{$brandbook->id}}/"><i
                                    class="far fa-eye"></i> {{__('View')}}</a>
                        @elseif($brandbook->bb_version == 3)
                            <a class="dropdown-item" href="{{route('brandbook.my')}}/view-new-v3/{{$brandbook->id}}/"><i
                                    class="far fa-eye"></i> {{__('View')}}</a>
                    ...
                    @if(Auth::user()->can_do('duplicate', $brandbook))
                        <form action="{{route('brandbook.duplicate', $brandbook->id)}}" method="post">
                            {{csrf_field()}}
                            {{method_field('PUT')}}
                            <button class="dropdown-item" href="{{route('brandbook.my')}}/duplicate/{{$brandbook->id}}" onclick="checkFirst(this)">
