...
<div class="gradient-theme">
	<div id="page1">
		<div class="logo">
			@php
				//replace all # colors
				$re = '/#([0-9a-fA-F]{3,6})/m'; 
				$logo = preg_replace($re, '#ffffff', $data->logo);

				//replace all rgb colors
				$re = '/rgb\([0-9 ]{1,10},[0-9 ]{1,10},[0-9 ]{1,10}\)/m';
				$logo = preg_replace($re, 'rgb(255,255,255)', $logo);

				//replace all rgba colors
				$re = '/rgba\([0-9 ]{1,10},[0-9 ]{1,10},[0-9 ]{1,10},([0-9. ]{1,10})\)/m';
				$logo = preg_replace($re, 'rgba(255,255,255,$1)', $logo);

				//check if there's no fill/stroke
				$re = '/(fill:)|(fill=")|(fill=\')|(fill :)|(stroke=")|(stroke:)|(stroke :)|(stroke=\')/m';
				if(!preg_match($re, $logo)){
					$re = '/<svg /m';
					$logo = preg_replace($re, '<svg fill="#fff" ', $logo);
				}

				//replace all ids with svgid (to protect gradient)
				$re = '/(SVGID_[0-9]{1,2})/mU';
				$logo = preg_replace($re, 'gradient-home-$1', $logo);

				//find all class_names
				$re = '/class="(.*)" /mU';
				preg_match_all($re, $logo, $matches, PREG_SET_ORDER, 0);
				foreach($matches as $match){
					$match = $match[1];
					$class_names = explode(' ', $match);
					foreach($class_names as $i=>$cn){
						$re = '/\.'.$cn.'/mU';
						$logo = preg_replace($re, '.gradient-home-'.$cn, $logo);
						$class_names[$i]='gradient-home-'.$cn;
					}
					$re = '/class="'.$match.'"/mU';
					$logo = preg_replace($re, 'class="'.join(' ', $class_names).'"', $logo);
				}
	...
	@if(count($data->approved)>0)
		...
		<div id="page15">
			@php
				$page_number++;
			@endphp
			<div class="page-number right @if(isset($data->approved[3]) && $data->approved[3]->background!='transparent' && $data->approved[3]->background!='#ffffff') text-white @endif">{{$page_number}}</div>
			<div class="logo-versions">
				@php
					$approved_count = 1;
				@endphp
	...
	<div id="page21">
		...
		<div class="page-number right">{{$page_number}}</div>
				...
				@php
					...
					if($data->logo_w<$data->logo_h){
						...
						if($longer>140){
							$longer = 140;
							$shorter = $shorter*140/$data->logo_w;
							$data->logo_h = 140;
							$data->logo_w = $shorter;
						}
						$shorter_text = 'x';
				...
				@endphp
			<div class="view-block" style="position: relative;top:{{($data->logo_h>0?$data->logo_h/2+$free_space_y_h:1)}}px;@if($data->logo_w/$data->logo_h<1 && $data->logo_h>=100)transform:scale(.8)@endif">
	   ...
	   "> 
		...
		@endphp
		<div class="icon iconster" style="width: {{$data->icon_w}}px; height: {{$data->icon_h}}px;margin-left: -{{$data->icon_w/2}}px;bottom: {{60+20}}px; {{--top: {{$logo_fsy+60}}px--}}">
	...
	<div id="page26">
		@php
			$page_number++;
		@endphp
		<div class="page-number" id="Color Pallete">{{$page_number}}</div>
			...
			{{-- {{dd($color)}} --}}
				...
				<div class="color-block">
					...
					<svg width="231" height="155" viewBox="0 0 231 155" fill="none" xmlns="http://www.w3.org/2000/svg">
						...
						@if(isset($color->color->scheme->colors[4]))
						<div class="color-title ct1">{{adjustBrightness($color->color->hex->value, 153)}}</div>
						@endif
						@if(isset($color->color->scheme->colors[3]))
						<div class="color-title ct2">{{adjustBrightness($color->color->hex->value, 76)}}</div>
						@endif
						@if(isset($color->color->scheme->colors[1]))
						<div class="color-title ct3">{{$color->color->hex->value}}</div>
						@endif
						@if(isset($color->color->scheme->colors[0]))
						<div class="color-title ct4">{{adjustBrightness($color->color->hex->value, -76)}}</div>
						@endif
		...
		@if(isset($data->fonts_main[1]))
			...
			@if(isset($data->colors_list[3]))
				<div class="huge-text text-four">
					<div style="overflow: hidden; height: 52px; width: 100%;">{{$data->fonts_main[1]['font_face']}}</div>
					<small>
		...
		<div class="company">
			...
			<div class="web">
				...
				@if(!empty($data->user->company_dribble))
					<p><i class="fa fa-dribbble" style="margin-right: 5px; display: inline-block;"></i>{{$data->user->company_dribble}}</p>
				@endif
				@if(!empty($data->user->company_behance))
					<p><i class="fa fa-behance" style="margin-right: 5px; display: inline-block;"></i>{{$data->user->company_behance}}</p>
				@endif
			</div>
			<div class="contact">
				<p>Contact</p>
				@if(!empty($data->user->company_phone))
					<p><i class="fa fa-mobile" style="margin-right: 5px; display: inline-block;"></i>{{$data->user->company_phone}}</p>
				@endif
				@if(!empty($data->user->email))
					<p><i class="fa fa-envelope" style="margin-right: 5px; display: inline-block;"></i>{{$data->user->email}}</p>
				@endif
			</div>
		</div>
	</div>
