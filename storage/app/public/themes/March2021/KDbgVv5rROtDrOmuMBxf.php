...
<head>
    ...
    @if(!empty($data->custom['body_family']))
        <link href="https://fonts.googleapis.com/css?family={{$data->custom['body_family']}}:{{$data->custom['body_weight']}}&display=swap"
              rel="stylesheet">
    @endif

    @if(!empty($data->custom['title_family']))
        <link href="https://fonts.googleapis.com/css?family={{$data->custom['title_family']}}:{{$data->custom['title_weight']}}&display=swap"
              rel="stylesheet">
    @endif
    ...
    @if(count($data->approved)>0 || count($data->logo_versions) > 1)
        <div id="page14">
            ...
            <h1 class="title" contenteditable="true" @blur="title_changed()"
                data-title-field="versions_title">{!! !empty($data->versions_title) ?
$data->versions_title :
            ...
            @if(count($logo_chunk) > 0)
                <div id="page16-1" class="{{count($logo_chunk) > 3 ? '' : 'page16-oneside'}}">
                    ...
                    @if(count($logo_chunk) > 3)
                        <div class="logo-versions-block">@endif
...
<div id="page21">
    ...
    <div class="view-block space-{{$data->space}}"
         ...
         @if(empty($data->icon) || $data->icon=='skipped' || $data->icon=='[]') top: 50%; margin-top: -{{$data->logo_h/2}}px; @endif
             ">
        <div class="logogsspace space_x"
             style="background-image: @if($show_clear_icon) url({{$data->logo_b64}}) @else none @endif;
        ...
        <div class="logo" style="    position: absolute;
            ...
            @if(isset($data->logo_b64))
                <div class="proportions_logo logo-image"
                     style="width: {{$data->logo_w}}px;height: {{$data->logo_h}}px;background-image: url({{$data->logo_b64}})"></div>
            @endif
            @if($clear_space_text!=='')
    ...
    <div id="page25">
        ...
        @foreach($data->missuses as $missuse)
            <div class="rejected_item">
                <div class="logo-image"
                     style="@foreach($missuse->style as $key=>$value) {{from_camel_case_3($key)}}: @if($value=='rotate(45deg)') rotate(20deg) @else{{$value}}@endif;@endforeach background-image: url({{$missuse->logo_b64}})"></div>
...
box-shadow: 5.20389px 5.20389px 19.5146px rgba(0, 0, 0, 0.25);padding: 9px;">
                            @if($color->show_shades)
                                <div class="darker-color"
                                     style="background: {{adjustBrightness_3($color->color->hex->value, -30)}};border-bottom: 2px solid #fff;height: 30px;display: flex;font-size: 10px;align-items: center;align-content: center;justify-content: center;color: {{getContrastColor_3(adjustBrightness_3($color->color->hex->value, -30))}};margin-bottom: 2px;">
                                    {{adjustBrightness_3($color->color->hex->value, -30)}}
                                </div>
                            @endif
                            <div class="main-color"
                                 style="background: {{$color->color->hex->value}};border-bottom: 2px solid #fff;height: 120px;display: flex;align-items: center;align-content: center;justify-content: center;font-size: 14px;color: {{getContrastColor_3($color->color->hex->value)}};margin-bottom: 2px;@if(!$color->show_shades) height: 216px; margin-bottom: 40px; @endif">
                            ...
                            @if($color->show_shades)
                                <div class="lightest-color"
                                     style="background: {{adjustBrightness_3($color->color->hex->value, 40)}};border-bottom: 2px solid #fff;height: 30px;display: flex;font-size: 10px;align-items: center;align-content: center;justify-content: center;color: {{getContrastColor_3(adjustBrightness_3($color->color->hex->value, 40))}};margin-bottom: 40px;">
...
<div id="page36">
    ...
    <div class="logo-gingersauce" style="color: #000;">
